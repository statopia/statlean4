...
| **很高**（全局重构/sSup） | **不委派**。主会话直接用 Edit 写代码（已有上下文），或拆成子任务 | esseen_smoothing_ineq |
...
**很高复杂度**: 两种处理方式：
1. **主会话直接写代码** — 当主会话已理解完整上下文且 3+ agent 失败时
   - **限制**：在 `/tmp/test_proof.lean` 中写好完整证明，一次性 paste 到目标文件
   - **禁止**在主会话中做 build-fix 循环（≤ 1 次 build）
   - 如果 build 失败 → 把半成品 + 错误信息委派给 agent，不在主会话调试
   - 原因：主会话 build 循环会快速消耗上下文，压缩后丢失积累的理解
2. **拆成 2-3 个中等子任务** — 当任务可分解时

**判断标准**：不预判复杂度。所有任务先按"中"处理。
Agent 返回后根据结果调整：
- 完成且 ≤ 1 build → 记录为"低"到 proof_knowledge.yaml 的 workflow 字段
- 完成但 >5 build → 记录为"高"，下次提供更多信息
- 未完成 → 拆子任务或主会话写（限 1 次 build）
