...
**当前规模**：36 个 Lean 文件，~180 个声明，28 个零 sorry 模块（其中 22 个已入库 `Verified.lean`）。
...
**9 个真实数学 sorry** 正等待攻击（详见 `theme/input/sorry_backlog.yaml`）。
...
# 整份 PDF 一站式
> /prove theme/input/raw/lecture-9-handout.pdf
...
# 也可以使用命令
> /prove theme/input/raw/lecture-9-handout.pdf的 Scheffé 定理
...
## 快速开始：Codex CLI 会话内使用

> Codex 遵循 `AGENTS.md` + `theme/formalize_playbook.md`。每步会写 checkpoint log 供验证。

### 从 PDF 到形式化

```bash
# 整份 PDF 一站式（Make pipeline）
codex exec --full-auto "Read AGENTS.md and theme/formalize_playbook.md first. \
  AGENT_BACKEND=codex make -C theme pdf-formalize PDF=lecture-9-handout.pdf"

# 只要 PDF 中的某个定理
codex exec --full-auto "Read AGENTS.md and theme/formalize_playbook.md first. \
  Follow Steps 0-7 exactly, writing checkpoint log to theme/out/formalize_checkpoint.jsonl after each step. \
  形式化 theme/input/raw/lecture-5-handout.pdf 里的 MLE 概念"
...
```bash
codex exec --full-auto "Read AGENTS.md and theme/prove_playbook.md first. \
  Attack the sorry in Statlean/Gaussian/Poincare.lean: condExp_eq_fiberAvg_pi. \
  Follow prove_playbook.md decision tree."
```

### 验证 Codex 是否遵循了 playbook

```bash
# 一键测试（清理 → 跑 Codex → 检查 log → 检查 Lean → build）
bash theme/tests/test_formalize_codex.sh

# 或手动检查 checkpoint log
...
```
Steps completed: 8/8
Declarations: likelihood, IsMLE, isMLE_comp
...
> **提示**：Codex prompt 里必须包含 `"Read AGENTS.md and theme/formalize_playbook.md first"` + `"writing checkpoint log to theme/out/formalize_checkpoint.jsonl after each step"`，否则 Codex 大概率不会读 playbook 也不会写 log。
...
# Codex
codex exec --full-auto "Read AGENTS.md and theme/formalize_playbook.md first. \
  Follow Steps 0-7, write checkpoint log. 形式化 lecture-9-handout.pdf 里的 Scheffé 定理"
```
...
make -C theme pdf-formalize PDF=path/to/paper.pdf                # 默认 claude
AGENT_BACKEND=codex make -C theme pdf-formalize PDF=lecture-6     # 用 codex
...
# 2. 提取 + 识别
make -C theme from-pdf PDF=lecture-5-handout.pdf
make -C theme from-tex
...
| **Codex CLI 会话** | 替代 agent，有 checkpoint 验证 | `codex exec --full-auto "..."` |
| **终端 Make** | CI / 批量处理 | `make -C theme pdf-formalize PDF=...` |
| **终端一句话** | 快速一次性任务 | `claude -p "..."` 或 `codex exec "..."` |
...
> 会话内使用见上方「快速开始：Codex CLI」。本节是 Make pipeline 的进阶用法。

```bash
# 最小命令组（给定 PDF -> 定向 prove -> gate）
AGENT_BACKEND=codex make -C theme pdf-formalize PDF=lecture-6
MANIFEST=out/manifest.json AGENT_BACKEND=codex AUTO_AGENT=1 MAX_ITERS=10 \
  MAX_PARALLEL=3 PROVE_BUDGET=3600 make -C theme prove-fallback
make -C theme gate

# 查看产物
test -f theme/out/manifest.json && echo "ok"
python3 -c "
import json; from collections import Counter
m = json.load(open('theme/out/manifest.json'))
print(Counter(e.get('status','?') for e in m.get('entries',{}).values()))
"
```

> Codex 读取 `AGENTS.md` + `theme/formalize_playbook.md`。`prove-fallback` 需传 `MANIFEST` 以限定攻击范围。
