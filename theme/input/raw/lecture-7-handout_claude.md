Looking through these slides, I can identify the following formal mathematical content:
...
$$\operatorname{Cov}\left(T(X), \frac{\partial}{\partial\theta} \log f_\theta(X)\right)^2 \leq \operatorname{Var}[T(X)]\operatorname{Var}\left[\frac{\partial}{\partial\theta} \log f_\theta(X)\right]$$

- By Eq (1), $E_\theta \frac{\partial}{\partial\theta} \log f_\theta(X) = 0$. Thus 
$$\operatorname{Var}\left[\frac{\partial}{\partial\theta} \log f_\theta(X)\right] = E\left[\left(\frac{\partial}{\partial\theta} \log f_\theta(X)\right)^2\right] = I(\theta)$$

- $\operatorname{Cov}\left(T, \frac{\partial}{\partial\theta} \log f_\theta(X)\right) = E\left[T \frac{\partial}{\partial\theta} \log f_\theta(X)\right] = \int T(x) \frac{\partial}{\partial\theta} f_\theta(x)d\nu$

- By Eq (2), the last display $= \frac{\partial}{\partial\theta} E[T] = g'(\theta)$

- We conclude that
$$g'(\theta)^2 \leq \operatorname{Var}(T) I(\theta)$$

## Theorem 3.3 (Multivariate C-R Lower Bound)
...
### Proof (Sketch)
- By the covariance inequality, for any $c \in \mathbb{R}^k$,
$$\operatorname{Var}(T)\operatorname{Var}\left(c^T \frac{\partial \log f_\theta(X)}{\partial\theta}\right) \geq \left[\operatorname{Cov}\left(T, c^T \frac{\partial \log f_\theta(X)}{\partial\theta}\right)\right]^2.$$

- Use $\mathbf{a} = \operatorname{Cov}\left(\frac{\partial}{\partial\theta} \log f_\theta(X), T(X)\right)$ to simplify notations
- The LHS is $\operatorname{Var}(T)$ $(c^T I(\theta)c)$
- The RHS is $(c^T \mathbf{a})^2$
- Choose $c = [I(\theta)]^{-1}\mathbf{a}$
- Use the regularity condition, we can show $\mathbf{a} = \frac{\partial}{\partial\theta} g(\theta)$
...
### Proof
• The p.d.f. under the natural parameter $\eta$ is
...
### Proof
• Since $\psi = \mathbb{E}[T(X)] = \frac{\partial}{\partial\eta}\zeta(\eta)$,
$$I(\eta) = \frac{\partial\psi^T}{\partial\eta} I(\psi) \left(\frac{\partial\psi^T}{\partial\eta}\right)^T = \frac{\partial^2}{\partial\eta\partial\eta^T}\zeta(\eta) I(\psi) \left[\frac{\partial^2}{\partial\eta\partial\eta^T}\zeta(\eta)\right]^T.$$

• By Theorem 2.1 (of the textbook) (see also Exercise 2 in Tutorial 4) and the result in (ii),
$$\frac{\partial^2}{\partial\eta\partial\eta^T}\zeta(\eta) = \operatorname{Var}(T) = I(\eta).$$

• Hence
$$I(\psi) = [I(\eta)]^{-1} I(\eta) [I(\eta)]^{-1} = [I(\eta)]^{-1} = [\operatorname{Var}(T)]^{-1}.$$