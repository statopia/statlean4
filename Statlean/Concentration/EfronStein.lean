...
**Proof sketch** (induction on `n = Fintype.card ι`):
Base n = 0: Both sides are 0 (empty sum; f is constant on the one-element space).
Base n = 1: Follows from `efron_stein_unique`.
Step n → n+1: Fix coord 0. Let g(x') = E_{μ₀}[f(·,x')] (marginalize over X₀).
  1. LTV for coord 0: Var[f] = E[Var[f|G_0^except]] + Var[g]
  2. IH for g on n coords: Var[g] ≤ Σᵢ≥1 E[Var[g|G_i^except_in_rest]]
  3. Jensen step: E[Var[g|G_i^except_in_rest]] ≤ E[Var[f|G_i^except_in_full]]
     (by Jensen on the map h ↦ ∫ h dμ₀, using `sq_integral_le_integral_sq`)
  4. Combine to get Var[f] ≤ Σᵢ₌₀ⁿ E[Var[f|G_i^except]].
-/
