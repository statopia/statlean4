...
/-! # Efron-Stein Inequality — Proved (sorry-free) declarations

This file contains all declarations from the Efron-Stein development that are
fully proved (zero sorry).  The remaining sorry-bearing declarations live in
`Statlean.Concentration.EfronStein`, which re-imports this file.
-/
...
theorem variance_marginals_le_variance_prod
  ...
  -- L² memberships (from proved lemmas)
