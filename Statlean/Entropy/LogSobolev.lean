...
lemma hasDerivAt_sq_of_hasDerivAt {f f' : ℝ → ℝ} (hderiv : ∀ x, HasDerivAt f (f' x) x) (x : ℝ) :
  ...
  -- Need to simplify: ↑2 * f x ^ 1 * f' x = 2 * f x * f' x
  -- and (f ^ 2) to (fun x => f x ^ 2)
  convert h using 1 <;> (simp [pow_succ, pow_zero])
...
/-! ## Proof decomposition for gaussian_lsi_1d_core

The main sorry `gaussian_lsi_1d_core` has type `SatisfiesLSI stdGaussian 2`.
Unfolding, this means: for all f, f' with MemLp f 2, MemLp f' 2, HasDerivAt f f',
  `entropy stdGaussian (fun x => f x ^ 2) ≤ 2 * ∫ x, f' x ^ 2 ∂stdGaussian`

where `entropy μ g = ∫ g·log(g) dμ - (∫ g dμ)·log(∫ g dμ)`.

We decompose this into sub-lemmas below. Each is marked with its sorry status.
-/

section LSI_Decomposition

/-- **Sub-lemma 1**: For the Gaussian, f ∈ L²(γ) and HasDerivAt f f' implies
    the entropy `∫ f²·log(f²) dγ` is well-defined (the integrand is in L¹).

    This requires controlling the growth of `x² · |log(x²)|` against Gaussian tails.
    Key: for the Gaussian, f·f' ∈ L¹ by Holder, and the log-Sobolev integrand
    grows at most like `|f|^{2+ε}` which is controlled by the Gaussian tail. -/
...
/-- **Sub-lemma 2 (IBP core)**: The key identity obtained from integration by parts
    with the Gaussian density. For C¹ functions f with f, f' ∈ L²(γ):

    `∫ f²·log(f²) dγ ≤ 2·∫ f'² dγ + (∫ f² dγ)·log(∫ f² dγ)`

    This is exactly `Ent(f²) ≤ 2·∫ f'²`, which IS the LSI.

    The proof uses the key insight: apply Stein/IBP to `h(x) = f(x)·log|f(x)|`
    (or a regularized version), yielding:
    `∫ x·f·log|f| dγ = ∫ (f'·log|f| + f'/f·f) dγ = ∫ (f'·log|f| + f') dγ`
    Combined with the entropy decomposition, this gives the bound.

    The regularization handles the issue at f(x) = 0 where log|f| is undefined.
    Standard approach: replace log|f| by log(|f| + ε), take ε → 0. -/
