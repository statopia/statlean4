...
every other sufficient statistic: for any sufficient S (into any measurable
space β), there exists measurable ψ such that T = ψ ∘ S  P-a.s. for all θ.
PIPELINE_ID: lec4.minimal_sufficiency_def -/
def IsMinimalSufficient' {Ω α : Type*} [MeasurableSpace Ω]
    [MeasurableSpace α]
    (P : ParametricFamily Θ Ω) (T : Ω → α) : Prop :=
  IsSufficient' P T ∧
  ∀ (β : Type*) [MeasurableSpace β] (S : Ω → β), IsSufficient' P S →
